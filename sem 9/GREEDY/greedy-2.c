...
//Problema rucsacului in varianta intreaga
//=>Se ocupa intreaga capacitate a rucsacului
//obiectele adaugate in rucsac NU pot fi fractionate
//Pentru un algoritm eficient obiectele sunt deja sortate descrescator dupa castigul adus de fiecare obiect.
//q - capacitatea totala a rucsacului
//n - nr. obiecte
//ob - lista de obiecte
//sol - vector care marcheaza obiectele introduse in rucsac
void rucsacIntreg(float q, int n, Obiect* ob, float* sol) {
	int i;

	//parcurgem pana trecem prin toate obiectele din lista
	for (i = 0; i < n && q > 0; i++) {
		if (q >= ob[i].cost) {
